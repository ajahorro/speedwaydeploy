-- ============================================================================
-- Fix create_booking_atomic(): cast payment_status JSON text -> booking_payment_status
-- ============================================================================
--
-- DEFECT
-- ------
-- The original create_booking_atomic() (20261001000001) inserts the master
-- booking with:
--
--     coalesce(v_booking ->> 'payment_status', 'unpaid'),
--
-- `->>` returns JSON as TEXT, and coalesce(text, text) is TEXT. The target
-- column `public.bookings.payment_status` is an ENUM (`booking_payment_status`),
-- so the INSERT fails with:
--
--     column "payment_status" is of type booking_payment_status
--     but expression is of type text
--
-- because PostgreSQL does not implicitly cast text -> enum inside an
-- INSERT ... SELECT ... column list. Every customer/admin booking that reached
-- the RPC therefore aborted, which is the reported "atomic booking type crash".
--
-- FIX
-- ---
-- 1. Explicitly cast the JSON text to the enum: `...::booking_payment_status`.
-- 2. Guard against an out-of-enum value arriving from an older client: map any
--    unknown value to the safe default 'unpaid' instead of raising 22P02.
--
-- `status` (bookings.status) is a plain TEXT column, so it needs no cast — it is
-- left exactly as-is. The rest of the function body is preserved verbatim from
-- 20261001000001 so this migration is a surgical, value-preserving replacement.
-- ============================================================================
