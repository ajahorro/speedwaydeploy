-- ============================================================================
-- Fix create_booking_atomic(): also cast bookings.vehicle_type -> vehicle_type enum
-- ============================================================================
--
-- Companion to 20261016000001 / 20261016000002. Those fixed payment_status,
-- payment_method and payments.payment_type. A live probe then surfaced the SAME
-- class of bug on one more column:
--
--     column "vehicle_type" is of type vehicle_type but expression is of type text
--
-- `bookings.vehicle_type` is the enum `vehicle_type`, whose ONLY valid members are
-- lowercase: sedan | suv | van | motorcycle. Clients (and any future caller) send
-- display labels such as 'Sedan', 'SUV', 'Van/L300', 'Regular', 'Bigbike', so this
-- migration normalises the label to the enum member and, on anything it cannot
-- map, stores NULL (the column is nullable) rather than aborting the booking.
--
-- This migration re-declares the FULL corrected function (superset of 0002), so a
-- database that only ever applies this one still ends up correct.
-- ============================================================================
--
...
begin
  ...
  -- Resolve the master booking's vehicle_type. bookings.vehicle_type is the enum
  -- `vehicle_type` (sedan | suv | van | motorcycle). Map the shop's display label
  -- to the enum member; anything unrecognised becomes NULL.
  ...
  select
    ...
    v_vehicle_type,                                      -- strictly typed enum (or null)
