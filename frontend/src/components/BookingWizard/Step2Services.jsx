          ...
          <div className="booking-unit-columns" style={{ opacity: complete ? 1 : .45, pointerEvents: complete ? 'auto' : 'none' }}>
                  ...
                  >
                    ...
                    <span style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '.4rem', flexWrap: 'wrap' }}>
                        ...
                        {hasDiscount && (
                          <span style={{ background: '#059669', color: '#fff', fontSize: '.62rem', padding: '.12rem .38rem', borderRadius: '3px', fontWeight: '900', textTransform: 'uppercase', letterSpacing: '.4px' }}>
                            {promoInfo.tagText}
                          </span>
                        )}
                      </div>
            ...
            <aside className="booking-unit-column" style={{ background: 'rgba(var(--admin-brand-rgb), .025)', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div key={service.id} style={{ display: 'flex', gap: '.5rem', justifyContent: 'space-between', color: 'var(--admin-text-primary)', fontSize: '.78rem' }}>
                      <span>
                        ...
                        {service.applied_promo && (
                          <span style={{ marginLeft: '.35rem', color: '#10b981', fontSize: '.7rem', fontWeight: '700' }}>
                            ({service.applied_promo})
                          </span>
                        )}
                      </span>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '.4rem' }}>
                        ...
                        <button type="button" onClick={() => toggleService(unit, service)} aria-label={`Remove ${service.name}`} style={{ color: '#ef4444', background: 'none', border: 0, cursor: 'pointer' }}><X size={15} /></button>
                      </div>
                    </div>
                  );
              ...
              <div style={{ marginTop: '1rem', paddingTop: '.85rem', borderTop: '1px solid var(--admin-border)' }}>
                ...
                {unitDiscount > 0 && (
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '.35rem', fontSize: '.75rem', color: '#10b981', fontWeight: '800' }}>
