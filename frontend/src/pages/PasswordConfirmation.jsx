...
const PasswordConfirmation = () => {
  ...
  return <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', background: 'var(--admin-bg)' }}>
    <div style={{ width: '100%', maxWidth: '440px', padding: '2.5rem', background: 'var(--admin-card)', border: '1px solid var(--admin-border)', borderRadius: '6px', color: 'var(--admin-text-primary)', textAlign: 'center' }}>
      ...
      {status === 'invalid' && <><p style={{ color: 'var(--admin-text-secondary)', lineHeight: 1.6 }}>This link has expired, was already used, or is invalid. Request a fresh one from the login screen — use <strong>Forgot Password?</strong>.</p><button onClick={requestNewLink} style={{ marginTop: '1rem', padding: '.75rem 1.25rem', background: 'var(--admin-brand)', color: 'var(--admin-text-on-brand)', border: 0, borderRadius: '4px', fontWeight: 800, cursor: 'pointer' }}>Request a New Link</button></>}
      {status === 'complete' && <><p style={{ color: 'var(--admin-text-secondary)', lineHeight: 1.6 }}>Your password has been updated successfully. You may now sign in with it.</p><button onClick={goToLogin} style={{ marginTop: '1rem', padding: '.75rem 1.25rem', background: 'var(--admin-brand)', color: 'var(--admin-text-on-brand)', border: 0, borderRadius: '4px', fontWeight: 800, cursor: 'pointer' }}>Continue to Login</button></>}
      {status === 'ready' && purpose === 'RESET' && <form onSubmit={submitReset} style={{ textAlign: 'left' }}><label style={{ display: 'block', marginBottom: '.4rem', fontWeight: 700 }}>New Password</label><input type="password" autoComplete="new-password" value={newPassword} onChange={event => setNewPassword(event.target.value)} required style={{ width: '100%', boxSizing: 'border-box', padding: '.8rem', background: 'var(--admin-bg)', color: 'var(--admin-text-primary)', border: '1px solid var(--admin-border)', borderRadius: '4px' }} /><label style={{ display: 'block', margin: '1rem 0 .4rem', fontWeight: 700 }}>Confirm New Password</label><input type="password" autoComplete="new-password" value={confirmPassword} onChange={event => setConfirmPassword(event.target.value)} required style={{ width: '100%', boxSizing: 'border-box', padding: '.8rem', background: 'var(--admin-bg)', color: 'var(--admin-text-primary)', border: '1px solid var(--admin-border)', borderRadius: '4px' }} /><button disabled={isSubmitting} style={{ width: '100%', marginTop: '1.25rem', padding: '.85rem', background: 'var(--admin-brand)', color: 'var(--admin-text-on-brand)', border: 0, borderRadius: '4px', fontWeight: 800, cursor: isSubmitting ? 'not-allowed' : 'pointer' }}>{isSubmitting ? 'Updating...' : 'Update Password'}</button></form>}
    </div>
  </div>;
};
